...
exports.createJobPost = catchAsyncErrors(async (req, res, next) => {
    ...
    if (!subscription) {
      return next(new ErrorHandler("No active subscription found. Please subscribe.", 403));
    }
  
    // 2. Count current job posts by the user
    const postedJobsCount = await Job.countDocuments({ userId });
  
    // 3. Compare with postLimit
    if (postedJobsCount >= subscription.postLimit) {
      return next(
        new ErrorHandler(
          `You have reached your job post limit (${subscription.postLimit}) for the ${subscription.planName} plan.`,
          403
        )
      );
    }
  
    // 4. Proceed to create job post
    const jobPost = await Job.create({
      userId,
      jobTitle,
      tags,
...
exports.getAllJobs = async (req, res) => {
    try {
        ...
        res.status(200).json({ success: true, totalActiveJobs: formattedJobs.length,
            jobs: formattedJobs });
...
exports.getRecentlyPostedJobs = async (req, res) => {
    try {
        const limit = parseInt(req.query.limit) || 5; // You can pass ?limit=10 from frontend
